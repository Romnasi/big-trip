...
export const createPointEditTemplate = (points) => {
  ...
  const pointPhotos = getPhotoList();
  ...
  const checkOfferAdded = (currentName) => {
  ...
  const getTypeOffers = () => {
    ...
    return offers[type] !== null
      ? Object.values(offers[type]).map(({name, price}) => `<div class="event__offer-selector">
        ...
        id="${nameWithoutSpace = getNameWithoutSpace(name)}-1" type="checkbox" name="${nameWithoutSpace}" ${checkOfferAdded(name) ? 'checked' : ''}>
